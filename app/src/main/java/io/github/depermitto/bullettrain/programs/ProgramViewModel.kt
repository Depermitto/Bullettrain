...
class ProgramViewModel(private val baseProgram: Program) : ViewModel() {
  ...
  fun setExercise(dayIndex: Int, exerciseIndex: Int, workoutEntry: WorkoutEntry) =
    setDay(
      dayIndex,
      getDay(dayIndex)
        .copy(entries = getDay(dayIndex).entries.smallListSet(exerciseIndex, workoutEntry)),
    )
  ...
  val hasChanged
    get() = getDays() != baseProgram.workouts
